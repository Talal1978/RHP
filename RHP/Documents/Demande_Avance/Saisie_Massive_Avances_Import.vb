...
Public Class Saisie_Massive_Avances_Import
    ...
    Friend frm_Avance As New Saisie_Massive_Avances
...
#End Region
    Sub PreparationTblCol()
        TblCol = DATA_READER_GRD("select  Table_Name, Colonne, Lib_colonne, Type_Data, Valeur_Default, Obligatoire, isnull(Cle,0) as Cle, Rang,Champs,Method_Champs as Affect, isnull(Rubrique,'') as Rubrique
                                from   Param_ModeleImportation where Cod_Modele='ImportAvance'")
    ...
    Private Sub BKW_Import_DoWork(sender As Object, e As DoWorkEventArgs) Handles BKW_Import.DoWork
        ...
        With Grd
            ...
            If orw.Length > 0 Then
                For i As Integer = 0 To orw.Length - 1
                    dupCol = orw(i)("Colonne")
                    Dim duplicates = CType(.DataSource, DataTable).AsEnumerable.GroupBy(Function(m) m.Field(Of String)(dupCol)).Where(Function(g) g.Count() > 1).Select(Function(g) g.Key)
                    For Each dup In duplicates
                        ShowMessageBox("Doublons intérdits pour la colonne  : " & dupCol & vbCrLf & dup.ToString)
                        NBError = 1
                        Exit Sub
                    Next
                Next
            ...
            For i = 0 To .RowCount - 2
                ...
                nRow = frm_Avance.Grd_Avance.DataSource.Select("Matricule='" & oMatricule & "'")
                If nRow.Length > 0 Then
                    For c = 0 To EVSel.Count - 1
                        nRow(0)("Avance") = IsNull(.Item("Montant_Avance", i).Value, 0)
                    Next
                End If
