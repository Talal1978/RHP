...
Public Class SuperGrd_Master
    Inherits DataGridViewX
#Region "Variables"
    ...
    Friend childView As New SuperGrd_Detail With {.Height = rowExpandedDivider - rowDividerMargin * 2, .Visible = False}
    '
...
#Region "Initialze and Display"
    Sub New()
        ...
        InitializeComponent()
        Me.ContextMenuStrip = AddContextMenu(False, True, True, False, False, False, False, False)
        applyGridTheme(Me)
        Dock = DockStyle.Fill
    End Sub
    Private Sub InitializeComponent()
        Me.components = New System.ComponentModel.Container()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(SuperGrd_Master))
...
#End Region
#Region "DataControl"
    Friend Sub setParentSource(ByVal tableName As DataTable, ByVal foreignKey As String)
        Me.DataSource = New DataView(tableName)
        setGridRowHeader(Me)
        _foreignKey = foreignKey
        If tableName.Columns(foreignKey).GetType.ToString = GetType(Integer).ToString _
        Or tableName.Columns(foreignKey).GetType.ToString = GetType(Double).ToString _
        Or tableName.Columns(foreignKey).GetType.ToString = GetType(Decimal).ToString _
        Then
