...
Public Class Zoom_Survey_Editeur_Formule
    ...
    Dim TblTitre As DataTable = DATA_READER_GRD("select Valeur,Membre from Param_Rubriques where Nom_Controle='Function' and left(Valeur,2) in ('FS','AB') and Valeur not in ('FS_Paie') order by Rang")
    Sub FunctionTrvMAJ()
        Building_Sys_RH_Agent_AG()
        If TblTrv.Rows.Count = 0 Then TblTrv = DATA_READER_GRD("select * from RH_Elements_Paie where Typ_Function in ('FS','AB') and isnull(nullif(id_Societe,-1)," & Societe.id_Societe & ")= " & Societe.id_Societe & xwhere & " order by  Cod_Function, Lib_Function")
        With TblTitre
    ...
    Private Sub Save_D_Click(sender As Object, e As EventArgs) Handles Save_pb.Click
        ...
        Try
            ...
            otxt.Value = Formule_Function_Text.Text
            Me.Close()
        Catch ex As Exception
suite:
            Resultat.Text = "#ERR : " & vbCrLf & Resultat.Text & vbCrLf & ex.Message
