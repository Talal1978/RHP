Public Class IR_Doctorants
    Dim titre As String = "Année de la déclaration : "
    Dim _Annee() As Integer = {-1}
    Dim SaisieLibre As Boolean = CnExecuting("select isnull((select top 1 isnull(Saisie_Libre,'false') from RH_Param_Declarations where id_Societe=" & Societe.id_Societe & " and Cod_Declaration='IR_Doctorants'),'false')").Fields(0).Value
...
WHERE     (Cod_Declaration = 'IR_Doctorants')
ORDER BY Rang")
    Sub Enregistrer()
        ...
        Try
            ...
            With Grd
                ...
                If swhere = "" Then
                    CnExecuting("delete from IR_Doctorants where id_Societe=" & Societe.id_Societe & " and Annee_Paie=" & _Annee(0))
                Else
                    CnExecuting("delete from IR_Doctorants where Clef not in (" & swhere & ") and id_Societe=" & Societe.id_Societe & " and Annee_Paie=" & _Annee(0))
                End If
                For i = 0 To .RowCount - IIf(.AllowUserToAddRows, 2, 1)
                    rs.Open("Select * from IR_Doctorants where Clef='" & IsNull(.Item("Clef", i).Value, -1) & "' and id_Societe=" & Societe.id_Societe & " and Annee_Paie=" & _Annee(0), cn, 2, 2)
                    If rs.EOF Then
                        rs.AddNew()
    ...
    Sub request()
        Try
            ...
            If _Annee(0) = -1 Then Return
            Dim nRow() As DataRow = Nothing
            Dim CodSql As String = "select  nom, prenom, adressePersonnelle, numCNI, numCE, mtBrutIndemnites, Clef
                                FROM IR_Doctorants
    ...
    Sub Requesting()
        If SaisieLibre Then
            ...
            With f
                .Combo_oModeleImport.SelectedValue = "IR_Doctorants"
                .swhere = "  where Cod_Modele = 'IR_Doctorants'  "
                newShowEcran(f, True)
            End With
        Else
            ...
            If Not IsNumeric(_Annee(0)) Then
                ...
                Return
            End If
            CnExecuting("exec Sys_Declaration " & Societe.id_Societe & ",'IR_Doctorants'," & _Annee(0) & ", ''")
        End If
        request()
    End Sub
    Sub Actualiser()
