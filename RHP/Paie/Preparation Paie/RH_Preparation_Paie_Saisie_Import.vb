...
Imports System.IO
Public Class RH_Preparation_Paie_Saisie_Import
    ...
    Friend frm_Paie As New RH_Preparation_Paie_Saisie
    Dim nomsColonnesOrigines As New ArrayList
    Friend _currentEP As String = ""
    ...
    Sub Request()
        ...
        With TblCol
            For i = 0 To .Rows.Count - 1
                ...
                With obtn
                    .BorderStyle = BorderStyle.None
                    .Font = New Font("Century Gothic", 9.0!, If(.Name = _currentEP, FontStyle.Underline, FontStyle.Regular), GraphicsUnit.Point, 0)
                    ...
                    .Text = TblCol.Rows(i).Item("Lib_colonne") & "  " & IIf(.Name = _currentEP, "✪", "")
...
#End Region
    ...
    Private Sub ButtonItem1_Click(sender As Object, e As EventArgs) Handles Modele_pb.Click
        Try
            ...
            Dim xlws As OfficeOpenXml.ExcelWorksheet = oWorkbook.Worksheets.Add("RHP_Paie")
            For Each c In pnlChamps.Controls
                If TypeOf c Is Label Then
                    nbCol += 1
                    For i = 1 To nbLig
...
#End Region
    Sub PreparationTblCol()
        ...
        Dim Dv = frm_Paie.TblEP.DefaultView
    ...
    Private Sub BKW_Import_DoWork(sender As Object, e As DoWorkEventArgs) Handles BKW_Import.DoWork
        ...
        With Grd
            ...
            For i = 0 To .RowCount - 2
                ...
                _Row = frm_Paie.TblPrePaie.Select("Matricule='" & oMatricule & "'")
                If _Row.Length = 0 Then
                    ShowMessageBox("Le Matricule : " & oMatricule & " ne correspondant pas aux agents sélectionnés pour cette opération d'import. " & i + 1)
                    NBError = 1
                    Exit Sub
                End If
            Next
            oAvancementStr = "Enregistrer"
            'Début d'Exportation
            For i = 0 To .RowCount - 2
                oMatricule = .Item("Matricule", i).Value
                _Row = frm_Paie.TblPrePaie.Select("Matricule='" & oMatricule & "'")
                For c = 0 To EVSel.Count - 1
                    nRow = frm_Paie.TblPrePaie.Select("Matricule='" & oMatricule & "' and Cod_Rubrique='" & EVSel(c) & "'")
                    If nRow.Length > 0 Then
                        nRow(0)("Valeur") = IsNull(.Item(EVSel(c), i).value, 0)
                    Else
                        frm_Paie.TblPrePaie.Rows.Add(oMatricule, _Row(0)("Nom"), EVSel(c), IsNull(.Item(EVSel(c), i).value, 0), frm_Paie.TblEP.Select($"Cod_Rubrique='{EVSel(c)}'")(0)("Typ_Rubrique"))
                    End If
    ...
    Private Sub BKW_Import_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles BKW_Import.RunWorkerCompleted
        ...
        Else
            For Each c In EVSel
                frm_Paie.TblEP.Select(" Cod_Rubrique='" & c & "'")(0)("Modified") = True
                frm_Paie.dicLbl(c).Text = frm_Paie._caractereSablier & " " & frm_Paie.TblEP.Select(" Cod_Rubrique='" & c & "'")(0)("Lib_Rubrique")
                If (c = _currentEP) Then
                    frm_Paie.elementPaieClicked(frm_Paie.dicLbl(c))
                End If
            Next
            frm_Paie.ReorganiserLbl()
            Me.Close()
        End If
    End Sub
