/* ============================================================================
   RHP - Module SP_ : ROLLBACK de la page portail "{{PAGE_TITLE}}"
   ----------------------------------------------------------------------------
   GENERE PAR le skill "rhp-portal-page-deployer".
   Annule le deploiement {{DEPLOY_FILE}} (demande {{CHANGE_REFERENCE}}).
   ----------------------------------------------------------------------------
   ...
   Limite documentee : pour une page modifiee (operation update), le rollback
   structurel consiste a re-executer le script de deploiement de la version
   precedente (regeneration deterministe) ou a restaurer la sauvegarde.
   ============================================================================ */

SET XACT_ABORT ON;
SET NOCOUNT ON;

DECLARE @DryRun         bit          = {{DRY_RUN}};
DECLARE @RemoveMetadata bit          = {{REMOVE_METADATA}}; -- 0 = desactivation seule
...
{{SECTION_ROLLBACK_BLOCK}}
    ...
    BEGIN
        COMMIT TRANSACTION;
        PRINT 'Rollback de ' + @CP + ' termine ({{CHANGE_REFERENCE}}).';
    END
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    PRINT 'Echec rollback {{PAGE_CODE}} : ' + LEFT(ERROR_MESSAGE(), 1000);
    ;THROW;
END CATCH;
GO
