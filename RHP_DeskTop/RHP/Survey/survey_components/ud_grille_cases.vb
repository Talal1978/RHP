    ...
    Overrides Sub Chargement()
        ...
        For i = 0 To col.Length - 1
            If col(i).Trim <> "" Then
                ...
                With rep
                    ...
                    .ReadOnly = True
                    .DefaultCellStyle.NullValue = My.Resources.RadioButtonUnsel
                End With
                Grd.Columns.Add(rep)
        ...
        For i = 0 To lig.Length - 1
            If lig(i).Trim <> "" Then
                ...
                End If
                Grd.Rows.Add("")
                Grd.Item(0, i).Value = lig(i) & IIf(Obligatoire, " (*)", "")
                If i = 0 Then laqst = lig(i)
                For j = 1 To Grd.ColumnCount - 1
                    Grd.Item(j, i).Value = IIf(repDic(CStr(i)).Split({";"}, StringSplitOptions.RemoveEmptyEntries)(j - 1).Trim = "1", My.Resources.RadioButtonSel, My.Resources.RadioButtonUnsel)
        ...
        Next
        Grd.Columns(0).HeaderText = If(laqst = laquestion, "", laquestion & IIf(Obligatoire And (laquestion.Trim <> ""), " (*)", ""))
        ...
        With Grd
            ...
            If e.RowIndex >= 0 And e.ColumnIndex >= 1 Then
                If .CurrentCell.Tag = False Then
                    .CurrentCell.Value = My.Resources.RadioButtonSel
                    .CurrentCell.Tag = True
                    .Item(0, e.RowIndex).Tag = e.ColumnIndex
                    For i = 1 To .ColumnCount - 1
                        If i <> e.ColumnIndex Then
                            .Item(i, e.RowIndex).Value = My.Resources.RadioButtonUnsel
                            .Item(i, e.RowIndex).Tag = False
                        End If
                    Next
                Else
                    .Item(e.ColumnIndex, e.RowIndex).Value = My.Resources.RadioButtonUnsel
            ...
            Else
                ...
                ElseIf Typ_Reponse = "cocher" Or Typ_Reponse = "echelle" Then
                    For j = 1 To .ColumnCount - 1
                        ...
                        End If
                    Next
                ElseIf modeScoring.Trim().ToLower() = "auto" Then
                    For i = 0 To .RowCount - 1
                        For j = 1 To .ColumnCount - 1
                            If .Item(j, i).Tag IsNot Nothing AndAlso CBool(.Item(j, i).Tag) Then
                                lesNotes.Add(j)
                            End If
                        Next
                    Next
                ...
                For i = 0 To lesNotes.Count - 1
                    If funcScoring <> "" Then
                        '  Dim noteFunc = Module_Generateur_Survey.myVBS.Eval($"Func_Survey_{codQuestion}({lesNotes(i)})")
                        '  If IsNumeric(noteFunc) Then lesNotes(i) = Math.Round(CDbl(noteFunc), 2)
                        Dim noteFunc = GetValeurFunction(i + 1)
                        If IsNumeric(noteFunc) Then lesNotes.Add(Math.Round(CDbl(noteFunc), 2))
                    End If
                Next
