Public Class Demande_Doc_Admin_Liste
    Friend swhere As String = " id_Societe=" & Societe.id_Societe
    Dim VenTbl As New DataTable
    Sub ChargementCombo()
        If Statut_Demande.Items.Count = 0 Then
            ' Assuming Statut_Demande uses 'Statut_Demande_Doc_Admin' rubric, or reuse 'Statut_Signature' if applicable
            ' For now, using 'Statut_Signature' as in Note_Frais, change if specific rubric exists
            Statut_Demande.fromRubrique("Statut_Signature")
            Statut_Demande.SelectedIndex = -1
        End If
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° Demande").Index Then
            Dim f As New Demande_Doc_Admin
            With f
                .Num_Demande_txt.Text = Ges_Pie_Clt_GRD.Item("N° Demande", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    Sub Requesting()
        ...
        End If
        If Statut_Demande.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(Statut,'') ='" & Statut_Demande.SelectedValue & "'"
        End If
        
        Cod_Sql = " SELECT Num_Demande as 'N° Demande', Matricule, Nom, Dat_Demande as 'Date', Commentaire, " &
                  " dbo.FindRubrique('Statut_Signature',Statut) as Statut, Etat_Traitement as 'Etat', isnull(Lib_Entite,'') as 'Entité' " &
                  ...
                  " where " & swhere & " Order by Dat_Demande desc"
    ...
    Sub Nouveau()
        Dim f As New Demande_Doc_Admin
        With f
            ' Initialize any defaults for new request if needed
            newShowEcran(f)
        End With
    End Sub
