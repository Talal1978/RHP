Public Class RH_Declaration_AT_Liste
    Friend swhere As String = " id_Societe=" & Societe.id_Societe
    Dim VenTbl As New DataTable

    Sub ChargementCombo()
        If Statut_AT.Items.Count = 0 Then
            Statut_AT.fromRubrique("Statut_Signature") ' Reuse simple status list or create specific one
            Statut_AT.SelectedIndex = -1
        End If
    End Sub

    Private Sub RH_Declaration_AT_Liste_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        If e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("Matricule").Index Then
            ...
            With f
                ...
                newShowEcran(f, True)
            End With
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° Déclaration").Index Then
            Dim f As New RH_Declaration_AT
            With f
                .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                .Num_Declaration_txt.Text = Ges_Pie_Clt_GRD.Item("N° Déclaration", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    ...
    Sub Requesting()
        ...
        If EstDate(Dat_Debut.Text) And EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Accident between '" & Dat_Debut.Text & "' and '" & Dat_Fin.Text & "' "
        ElseIf EstDate(Dat_Debut.Text) Then
            swhere = swhere & " and Dat_Accident >= '" & Dat_Debut.Text & "'"
        ElseIf EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Accident <= '" & Dat_Fin.Text & "'"
        End If
        If Statut_AT.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(Statut,'') ='" & Statut_AT.SelectedValue & "'"
        End If
        
        Cod_Sql = " SELECT Num_Declaration as 'N° Déclaration', Matricule, Nom, Dat_Accident as 'Date Accident', Lieu_Accident as 'Lieu', dbo.FindRubrique('Statut_Signature',Statut) as Statut, " &
                  ...
                  " where " & swhere & " Order by Dat_Accident desc"

        Dim Tbl0 As DataTable = DATA_READER_GRD(Cod_Sql)
        With Ges_Pie_Clt_GRD
            .DataSource = Tbl0
            .Columns("Date Accident").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
            If Matricule_txt.Text <> "" Then
                .Columns("Matricule").Visible = False
                .Columns("Nom").Visible = False
            End If
            Ges_Pie_Clt_GRD.setFilter({ .Columns("Matricule").Index, .Columns("Nom").Index, .Columns("Date Accident").Index, .Columns("Entité").Index})
        End With
        Cursor = Cursors.Default
    End Sub

    Private Sub Reset_Link_Click(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs)
        Reset_Form(Me)
    End Sub
    ...
    Private Sub ClearStatut_Link_Click(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LinkLabel15.LinkClicked
        Statut_AT.SelectedIndex = -1
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles Ges_Pie_Clt_GRD.CellMouseMove
        ...
        With Ges_Pie_Clt_GRD
            If e.ColumnIndex = .Columns("Matricule").Index Or e.ColumnIndex = .Columns("N° Déclaration").Index Then
                .Cursor = Cursors.Hand
            Else
                .Cursor = Cursors.Default
            End If
