Public Class RH_Demande_Avance_Liste
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° demande").Index Then
            If Ges_Pie_Clt_GRD.Item("Type", e.RowIndex).Value = "Demande" Then
                Dim f As New RH_Demande_Avance
                With f
                    .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                    .Num_Avance_txt.Text = Ges_Pie_Clt_GRD.Item("N° demande", e.RowIndex).Value
                    newShowEcran(f, True)
                End With
            End If
        End If
    ...
    Sub Requesting()
        ...
        End If
        Cod_Sql = " SELECT  case when isnull(Num_List_Avance,'')!='' then 'Saisie massive' else 'Demande' end as [Type],Num_Avance as 'N° demande', Matricule,Nom,dbo.FindRubrique('Statut_Signature',Statut) as Statut, Dat_Demande as 'Date', Montant_Avance as 'Montant demandé', Reglement 'Réglement', Commentaire, 
Traite as 'Traité', Paie_Calculee as 'Paie calculée', isnull(Lib_Entite,'') as 'Entité',isnull(Num_List_Avance,'') as 'N° liste'
...
where " & swhere & " Order by [Date] desc"
        ...
        With Ges_Pie_Clt_GRD
            ...
            .Columns("Réglement").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("Réglement").DefaultCellStyle.Format = "N2"
            If Matricule_txt.Text <> "" Then
            ...
            Ges_Pie_Clt_GRD.setFilter({ .Columns("Matricule").Index, .Columns("Nom").Index, .Columns("Date").Index,
                                .Columns("Montant demandé").Index, .Columns("Entité").Index, .Columns("Réglement").Index})
