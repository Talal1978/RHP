Public Class Note_Frais_Liste
    Friend swhere As String = " id_Societe=" & Societe.id_Societe
    Dim VenTbl As New DataTable
    Sub ChargementCombo()
        If Statut_NF.Items.Count = 0 Then
            Statut_NF.fromRubrique("Statut_Signature")
            Statut_NF.SelectedIndex = -1
        End If
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        If e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("Matricule").Index Then
            ...
            With f
                ...
                newShowEcran(f, True)
            End With
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° Note de frais").Index Then
            Dim f As New Note_Frais
            With f
                .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                .Num_NF_txt.Text = Ges_Pie_Clt_GRD.Item("N° Note de frais", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    Sub Requesting()
        ...
        If EstDate(Dat_Debut.Text) And EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and  Dat_NF between '" & Dat_Debut.Text & "' and '" & Dat_Fin.Text & "' "
        ElseIf EstDate(Dat_Debut.Text) Then
            swhere = swhere & " and Dat_NF >= '" & Dat_Debut.Text & "'"
        ElseIf EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_NF <= '" & Dat_Fin.Text & "'"
        End If
        If Statut_NF.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(Statut,'') ='" & Statut_NF.SelectedValue & "'"
        End If
        Cod_Sql = " SELECT  Num_NF as 'N° Note de frais', Matricule,Nom,isnull(Lib_NF,'') as Libellé, dbo.FindRubrique('Statut_Signature',Statut) as Statut, Dat_NF as 'Date', Mnt_NF as 'Montant engagé', Commentaire, 
Traite as 'Traité', Paie_Calculee as 'Paie calculée', isnull(Lib_Entite,'') as 'Entité'
...
where " & swhere & " Order by [Date] desc"
        ...
        With Ges_Pie_Clt_GRD
            ...
            .Columns("Montant engagé").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("Montant engagé").DefaultCellStyle.Format = "N2"
            If Matricule_txt.Text <> "" Then
            ...
            Ges_Pie_Clt_GRD.setFilter({ .Columns("Matricule").Index, .Columns("Nom").Index, .Columns("Date").Index,
                                .Columns("Montant engagé").Index, .Columns("Entité").Index})
    ...
    Private Sub Ges_Pie_Clt_GRD_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles Ges_Pie_Clt_GRD.CellMouseMove
        ...
        With Ges_Pie_Clt_GRD
            If e.ColumnIndex = .Columns("Matricule").Index Or e.ColumnIndex = .Columns("N° Note de frais").Index Then
                .Cursor = Cursors.Hand
            Else
                .Cursor = Cursors.Default
            End If
