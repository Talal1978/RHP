Public Class Zoom_Preparation_Paie_Add_Agent
    ...
    Private Sub Confirm_D_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Save_pb.Click
        With Grd
            ...
            If xwhere <> "" Then
                ...
                xwhere = "and (Matricule in (" & xwhere & "))"
                Dim TblEX_Base As DataTable = DATA_READER_GRD("select Matricule, Cod_Rubrique, Valeur from RH_Agent_Element_Paie e 
...
and id_Societe =" & Societe.id_Societe)
                ...
                f_preparation.PrePaie.InitialisationPaie(f_preparation.Cod_Preparation_Text.Text, f_preparation.Cod_Plan_Paie_Text.Text, f_preparation.Dat_Deb_Periode_Text.Text, f_preparation.Dat_Fin_Periode_Text.Text, f_preparation.withLog_chk.Checked)
                f_preparation.PrePaie.TblAgent = DATA_READER_GRD($"select * from {sql_Sys_RH_Agent_AG} where id_Societe=" & Societe.id_Societe & " and isnull(Cod_Plan_Paie,'')='" & f_preparation.Cod_Plan_Paie_Text.Text & "'")
                xwhere = f_preparation.PrePaie.ChargerNouvellePreparation(xwhere)
                Dim Tbl As DataTable = DATA_READER_GRD(xwhere)
                If f_preparation.PrePaie_Grd.RowCount = 0 Then
                    With f_preparation
                        .PrePaie.TblPrePaie = Tbl
                        .PrePaie.TblEX_Base = TblEX_Base
                        .MisEnFormeGrd()
                    End With
                Else
                    For i = 0 To Tbl.Rows.Count - 1
                        Dim dtrw As DataRow = f_preparation.PrePaie.TblPrePaie.NewRow
                        For Each c As DataGridViewColumn In f_preparation.PrePaie_Grd.Columns
                            If Tbl.Columns.Contains(c.Name) Then
                                dtrw(c.Name) = Tbl.Rows(i)(c.Name)
                            Else
                                dtrw(c.Name) = 0
                            End If
                        Next
                        f_preparation.PrePaie.TblPrePaie.Rows.Add(dtrw)
                    Next
                    For i = 0 To TblEX_Base.Rows.Count - 1
                        Dim dtrw As DataRow = f_preparation.PrePaie.TblEX_Base.NewRow
                        dtrw("Matricule") = TblEX_Base.Rows(i)("Matricule")
                        dtrw("Cod_Rubrique") = TblEX_Base.Rows(i)("Cod_Rubrique")
                        dtrw("Valeur") = TblEX_Base.Rows(i)("Valeur")
                        f_preparation.PrePaie.TblEX_Base.Rows.Add(dtrw)
                    Next
                End If
