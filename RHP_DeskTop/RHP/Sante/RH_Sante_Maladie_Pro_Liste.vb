Public Class RH_Sante_Maladie_Pro_Liste
    Friend swhere As String = " id_Societe=" & Societe.id_Societe

    Sub ChargementCombo()
        If Statut_Declaration_cbo.Items.Count = 0 Then
            Statut_Declaration_cbo.fromRubrique("Statut_Declaration_MP")
            Statut_Declaration_cbo.SelectedIndex = -1
        End If
    End Sub

    Private Sub RH_Sante_Maladie_Pro_Liste_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Sub Requesting()
        ...
        If Matricule_txt.Text <> "" Then
            swhere = swhere & " and m.Matricule ='" & Matricule_txt.Text & "' "
        End If
        If Cod_Entite_txt.Text <> "" And theUser.Matricule <> Matricule_txt.Text Then
            swhere = swhere & " and exists(select Matricule from RH_Agent where id_Societe=m.id_Societe and Matricule=m.Matricule and isnull(Cod_Entite,'') ='" & Cod_Entite_txt.Text & "')"
        End If
        If EstDate(Dat_Debut.Text) And EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Declaration between '" & Dat_Debut.Text & "' and '" & Dat_Fin.Text & "' "
        ElseIf EstDate(Dat_Debut.Text) Then
            swhere = swhere & " and Dat_Declaration >= '" & Dat_Debut.Text & "'"
        ElseIf EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Declaration <= '" & Dat_Fin.Text & "'"
        End If
        If Statut_Declaration_cbo.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(m.Statut_Declaration,'') ='" & Statut_Declaration_cbo.SelectedValue & "'"
        End If

        Dim Cod_Sql As String =
            " SELECT Num_MP as 'N° MP', m.Matricule, Nom, Dat_Declaration as 'Déclarée le', " &
            " m.Pathologie, dbo.FindRubrique('Statut_Declaration_MP',m.Statut_Declaration) as 'Statut', isnull(Lib_Entite,'') as 'Entité' " &
            " FROM RH_Sante_Maladie_Pro m " &
            " outer apply (select Nom_Agent + ' ' +Prenom_Agent as Nom, Cod_Entite from RH_Agent where id_Societe=m.id_Societe and Matricule=m.Matricule) r " &
            " outer apply (select Lib_Entite from Org_Entite where id_Societe=m.id_Societe and Cod_Entite=r.Cod_Entite) e " &
            " where " & swhere & " Order by Dat_Declaration desc"

        Dim Tbl0 As DataTable = DATA_READER_GRD(Cod_Sql)
        With Ges_Pie_Clt_GRD
            .DataSource = Tbl0
            If .Columns.Contains("Déclarée le") Then .Columns("Déclarée le").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
            If Matricule_txt.Text <> "" Then
                .Columns("Matricule").Visible = False
                .Columns("Nom").Visible = False
            End If
            Ges_Pie_Clt_GRD.setFilter({ .Columns("Matricule").Index, .Columns("Nom").Index, .Columns("Déclarée le").Index, .Columns("Entité").Index})
        End With
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        If e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("Matricule").Index Then
            ...
            With f
                ...
                newShowEcran(f, True)
            End With
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° MP").Index Then
            Dim f As New RH_Sante_Maladie_Pro
            With f
                .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                .Num_MP_txt.Text = Ges_Pie_Clt_GRD.Item("N° MP", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles Ges_Pie_Clt_GRD.CellMouseMove
        ...
        With Ges_Pie_Clt_GRD
            If e.ColumnIndex = .Columns("Matricule").Index Or e.ColumnIndex = .Columns("N° MP").Index Then
                .Cursor = Cursors.Hand
            Else
                .Cursor = Cursors.Default
            End If
