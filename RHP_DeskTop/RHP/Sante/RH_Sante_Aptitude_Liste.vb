Public Class RH_Sante_Aptitude_Liste
    ...
    Private Sub RH_Sante_Aptitude_Liste_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Sub Requesting()
        ...
        ChargementCombo()
        ' Sans fonction CLINIQUE : uniquement les fiches publiees et validees
        If Not Sante_FonctionActive("SANTE_CLINIQUE") Then
            swhere = swhere & " and isnull(a.Publie_RH,'false')='true' and isnull(a.Statut,'') in ('VA','SG')"
        End If
        ...
        If Cod_Entite_txt.Text <> "" And theUser.Matricule <> Matricule_txt.Text Then
            swhere = swhere & " and exists(select Matricule from RH_Agent where id_Societe=a.id_Societe and Matricule=a.Matricule and isnull(Cod_Entite,'') ='" & Cod_Entite_txt.Text & "')"
        End If
        If EstDate(Dat_Debut.Text) And EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Aptitude between '" & Dat_Debut.Text & "' and '" & Dat_Fin.Text & "' "
        ElseIf EstDate(Dat_Debut.Text) Then
            swhere = swhere & " and Dat_Aptitude >= '" & Dat_Debut.Text & "'"
        ElseIf EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Aptitude <= '" & Dat_Fin.Text & "'"
        End If
        If Statut_Aptitude_cbo.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(a.Statut_Aptitude,'') ='" & Statut_Aptitude_cbo.SelectedValue & "'"
        End If
        If Statut_cbo.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(a.Statut,'') ='" & Statut_cbo.SelectedValue & "'"
        End If

        Dim Cod_Sql As String =
            " SELECT Num_Aptitude as 'N° fiche', a.Matricule, Nom, Dat_Aptitude as 'Date', " &
            ...
            " a.Version, dbo.FindRubrique('Statut_Signature',a.Statut) as Statut, isnull(Lib_Entite,'') as 'Entité' " &
            " FROM RH_Sante_Aptitude a " &
            " outer apply (select Nom_Agent + ' ' +Prenom_Agent as Nom, Cod_Entite from RH_Agent where id_Societe=a.id_Societe and Matricule=a.Matricule) r " &
            " outer apply (select Lib_Entite from Org_Entite where id_Societe=a.id_Societe and Cod_Entite=r.Cod_Entite) e " &
            " where " & swhere & " Order by Dat_Aptitude desc"
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        If e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("Matricule").Index Then
            ...
            With f
                ...
                newShowEcran(f, True)
            End With
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N° fiche").Index Then
            Dim f As New RH_Sante_Aptitude
            With f
                .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                .Num_Aptitude_txt.Text = Ges_Pie_Clt_GRD.Item("N° fiche", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles Ges_Pie_Clt_GRD.CellMouseMove
        ...
        With Ges_Pie_Clt_GRD
            If e.ColumnIndex = .Columns("Matricule").Index Or e.ColumnIndex = .Columns("N° fiche").Index Then
                .Cursor = Cursors.Hand
            Else
                .Cursor = Cursors.Default
            End If
