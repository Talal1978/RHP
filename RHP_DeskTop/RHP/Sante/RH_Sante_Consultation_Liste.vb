Public Class RH_Sante_Consultation_Liste
    Friend swhere As String = " id_Societe=" & Societe.id_Societe

    Sub ChargementCombo()
        If Typ_Acte_cbo.Items.Count = 0 Then
            Typ_Acte_cbo.fromRubrique("Typ_Acte_Infirmier")
            Typ_Acte_cbo.SelectedIndex = -1
        End If
    End Sub

    Private Sub RH_Sante_Consultation_Liste_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Sub Requesting()
        ...
        If Matricule_txt.Text <> "" Then
            swhere = swhere & " and c.Matricule ='" & Matricule_txt.Text & "' "
        End If
        ...
        If EstDate(Dat_Debut.Text) And EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Consultation between '" & Dat_Debut.Text & "' and '" & Dat_Fin.Text & "' "
        ElseIf EstDate(Dat_Debut.Text) Then
            swhere = swhere & " and Dat_Consultation >= '" & Dat_Debut.Text & "'"
        ElseIf EstDate(Dat_Fin.Text) Then
            swhere = swhere & " and Dat_Consultation <= '" & Dat_Fin.Text & "'"
        End If
        If Typ_Acte_cbo.SelectedIndex >= 0 Then
            swhere = swhere & " and isnull(c.Typ_Acte,'') ='" & Typ_Acte_cbo.SelectedValue & "'"
        End If

        Dim Cod_Sql As String =
            " SELECT Num_Consultation as 'N°', c.Matricule, Nom, Dat_Consultation as 'Date', " &
            " dbo.FindRubrique('Typ_Acte_Infirmier',c.Typ_Acte) as 'Acte', " &
            " dbo.FindRubrique('Suite_Consultation',c.Suite) as 'Suite', isnull(Lib_Entite,'') as 'Entité' " &
            " FROM RH_Sante_Consultation c " &
            " outer apply (select Nom_Agent + ' ' +Prenom_Agent as Nom, Cod_Entite from RH_Agent where id_Societe=c.id_Societe and Matricule=c.Matricule) r " &
            " outer apply (select Lib_Entite from Org_Entite where id_Societe=c.id_Societe and Cod_Entite=r.Cod_Entite) e " &
            " where " & swhere & " Order by Dat_Consultation desc"
    ...
    Private Sub Ges_Pie_Clt_GRD_CellContentDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Ges_Pie_Clt_GRD.CellContentDoubleClick
        ...
        If e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("Matricule").Index Then
            ...
            With f
                ...
                newShowEcran(f, True)
            End With
        ElseIf e.ColumnIndex = Ges_Pie_Clt_GRD.Columns("N°").Index Then
            Dim f As New RH_Sante_Consultation
            With f
                .Matricule_txt.Text = Ges_Pie_Clt_GRD.Item("Matricule", e.RowIndex).Value
                .Num_Consultation_txt.Text = Ges_Pie_Clt_GRD.Item("N°", e.RowIndex).Value
                newShowEcran(f, True)
            End With
        End If
    End Sub
    ...
    Private Sub Ges_Pie_Clt_GRD_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles Ges_Pie_Clt_GRD.CellMouseMove
        ...
        With Ges_Pie_Clt_GRD
            If e.ColumnIndex = .Columns("Matricule").Index Or e.ColumnIndex = .Columns("N°").Index Then
                .Cursor = Cursors.Hand
            Else
                .Cursor = Cursors.Default
            End If
