...
async function applyFix() {
    try {
        ...
        console.log("Dropping SQL Function...");
        const dropSql = `IF OBJECT_ID('dbo.Sys_Portail_DashBoard_Insights', 'IF') IS NOT NULL DROP FUNCTION dbo.Sys_Portail_DashBoard_Insights`;
        let res = await lireSql(dropSql);
        console.log("Drop result:", res.result);

        console.log("Recreating SQL Function...");
        const createSql = `
        ...
        if (!res.result) console.error("Create Data:", res.data);

        // Immediate Verification
        console.log("Verifying Function Definition...");
        const verifySql = "select top 1 text from syscomments where id = object_id('dbo.Sys_Portail_DashBoard_Insights')";
        const verifyRes = await lireSql(verifySql);
        if (verifyRes.data && verifyRes.data.length > 0) {
            console.log("First line of definition in DB:", verifyRes.data[0].text);
            if (verifyRes.data[0].text.includes("Cod_Survey")) {
                console.log("SUCCESS: 'Cod_Survey' found in definition.");
            } else {
                console.log("FAILURE: 'Cod_Survey' NOT found in definition.");
            }
        } else {
            console.log("Could not retrieve definition.");
        }
        ...
        console.error("Failed to force fix:", e);
    }
}
