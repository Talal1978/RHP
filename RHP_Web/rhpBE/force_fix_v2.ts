...
async function applyFix() {
    try {
        ...
        // Try using ALTER directly with a unique comment marker
        const marker = "/* GEMINI_FIX_20260120 */";
        const sqlFix = `ALTER FUNCTION dbo.Sys_Portail_DashBoard_Insights
            (	
                @Pilote nvarchar(50),
                @idSoc int,
                @Top int = 5
            ...
            f.Cod_Survey as Cod_Survey ${marker}
        ...
        console.log("Executing ALTER FUNCTION...");
        const res = await lireSql(sqlFix);
        console.log("ALTER result:", res.result);
        if (!res.result) {
            console.error("ALTER failed:", res.sort); // Log the error
        }

        // Verify Checksum/Marker
        const verifySql = `select top 1 text from syscomments where id = object_id('dbo.Sys_Portail_DashBoard_Insights') and text like '%GEMINI_FIX_20260120%'`;
        const verifyRes = await lireSql(verifySql);
        if (verifyRes.data && verifyRes.data.length > 0) {
            console.log("SUCCESS: Marker found in function definition.");
        } else {
            console.log("FAILURE: Marker NOT found.");
            // Dump full text
            const dumpSql = `select text from syscomments where id = object_id('dbo.Sys_Portail_DashBoard_Insights')`;
            const dumpRes = await lireSql(dumpSql);
            if (dumpRes.data) {
                console.log("--- START EXISTING DEFINITION ---");
                dumpRes.data.forEach((r: any) => console.log(r.text));
                console.log("--- END EXISTING DEFINITION ---");
            }
        }
