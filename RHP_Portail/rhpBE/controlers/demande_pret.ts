...
export async function demande_pret_liste(req: Request, res: Response) {
  ...
  Statut = Statut || "";
  let sqlStr = `SELECT  Num_Demande_Pret as 'N° demande', ${
...
export async function get_demande_pret(req: Request, res: Response) {
  ...
  Statut FROM RH_Pret_Demande where  Num_Demande_Pret=@Num_Demande_Pret and id_Societe=${idSoc}`;
  return res.send(
    await lireSql(sqlStr, [
      {
        param: "Num_Demande_Pret",
        sqlType: NVarChar,
...
export async function save_demande_pret(req: Request, res: Response) {
  ...
  if (!Num_Demande_Pret || Num_Demande_Pret === "") {
    const rsNum = await lireSql(
      `select 'DP${id_Societe}-${new Date().getFullYear()}'+right('000000'+convert(nvarchar(6),isnull(max(racine),0)+1),6) as racine from (select convert(int,case when isnumeric(ISNULL(racine,''))!=1 then 0 else racine end ) as Racine from RH_Pret_Demande 
  ...
  const rsEnt = await ecrireSql({
    ...
    fields: { ...entete, Num_Demande_Pret, id_Societe },
  ...
  if (rsEnt.result) {
    if (entete.Statut === "SS")
      await sousmettre_signature("DP", Num_Demande_Pret, id_Societe, Matricule);
...
export async function delete_demande_pret(req: Request, res: Response) {
  ...
  const rsl = await lireSql(
    `delete from RH_Pret_Demande where Num_Demande_Pret=@Num_Demande_Pret and id_Societe='${req.params.id_Societe}'`,
    [{ param: "Num_Demande_Pret", sqlType: NVarChar, valeur: Num_Demande_Pret }]
  );
  if (rsl.result) {
