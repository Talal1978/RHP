/* ============================================================================
   ...
   (duplicatas des pages standards) :
     P2  SP_Page_Table.Source_Metier + Source_Mapping : détail VIRTUEL alimenté
         par une source (Typ_Retour='TABLE') - grille en lecture seule sans
         table physique (ex. découpe d'un congé par période de paie).
     P4  SP_Page.Figer_Statuts : liste CSV des statuts figeant le document
         (défaut 'SG,RJ,SP,VA' ; ex. 'SS,SG,RJ,SP,VA' pour figer dès soumission).
     P5  SP_Page_Champ.Zoom_Condition : condition de zoom avec placeholders
...
IF COL_LENGTH('dbo.SP_Page_Champ', 'Zoom_Condition') IS NULL
    ALTER TABLE dbo.SP_Page_Champ
        ADD Zoom_Condition nvarchar(500) NULL;

IF COL_LENGTH('dbo.SP_Page_Table', 'Source_Metier') IS NULL
    ALTER TABLE dbo.SP_Page_Table
        ADD Source_Metier nvarchar(50) NULL;

IF COL_LENGTH('dbo.SP_Page_Table', 'Source_Mapping') IS NULL
    ALTER TABLE dbo.SP_Page_Table
...
SELECT 'SP_Page.Figer_Statuts' AS Colonne,
       CASE WHEN COL_LENGTH('dbo.SP_Page','Figer_Statuts') IS NULL THEN 'KO' ELSE 'OK' END AS Etat
UNION ALL SELECT 'SP_Page_Champ.Zoom_Condition',
       CASE WHEN COL_LENGTH('dbo.SP_Page_Champ','Zoom_Condition') IS NULL THEN 'KO' ELSE 'OK' END
UNION ALL SELECT 'SP_Page_Table.Source_Metier',
       CASE WHEN COL_LENGTH('dbo.SP_Page_Table','Source_Metier') IS NULL THEN 'KO' ELSE 'OK' END
UNION ALL SELECT 'SP_Page_Table.Source_Mapping',
       CASE WHEN COL_LENGTH('dbo.SP_Page_Table','Source_Mapping') IS NULL THEN 'KO' ELSE 'OK' END;
GO
