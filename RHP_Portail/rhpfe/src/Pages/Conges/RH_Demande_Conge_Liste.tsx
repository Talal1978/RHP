...
const RH_Demande_Conge_Liste = () => {
  const navigate = useNavigate();
  const { criteres, stateChange, ds, setDs, dsFields, setDsFields } =
    useEtatListe<TCriteres>("RH_Demande_Conge_Liste", initialiserCriteres);
        ...
        <>
          ...
          >
            <Bouton
              ...
              onClick={() => {
                myAxios("demande_conge_liste", criteres)
                  .then((dt) => {
      ...
      >
        <Grille
          ...
          onclick={({ colIndex, value }) => {
            if (colIndex === 0) {
              navigate(`../myspace/RH_Demande_Conge/Demande de congé/${value}`);
            }
          }}
          sx={{
...
export default RH_Demande_Conge_Liste;
type TCriteres = {
  Matricule?: string;
  Cod_Entite?: string;
  Statut?: string;
