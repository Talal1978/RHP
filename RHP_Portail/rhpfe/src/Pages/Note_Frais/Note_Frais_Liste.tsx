...
const Note_Frais_Liste = () => {
  const navigate = useNavigate();
  const { criteres, stateChange, ds, setDs, dsFields, setDsFields } =
    useEtatListe<TCriteres>("Note_Frais_Liste", initialiserCriteres);
        ...
        <>
          ...
          >
            <Bouton
              ...
              onClick={() => {
                myAxios("note_frais_liste", criteres)
                  .then((dt) => {
      ...
      >
        <Grille
          ...
          onclick={({ colIndex, value }) => {
            if (colIndex === 0) {
              navigate(`../myspace/Note_Frais/Note de frais/${value}`);
            }
          }}
          sx={{
...
export default Note_Frais_Liste;
type TCriteres = {
  Matricule?: string;
  Cod_Entite?: string;
  Statut?: string;
