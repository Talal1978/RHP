...
const Demande_Pret = () => {
  ...
  function stateChange(champs: string, valeur: any) {
    if (champs === "Num_Demande_Pret" && currentNum !== valeur) {
  ...
  useEffect(() => {
    Request();
    setSignatureProps({ typ_document: "DP", valeur_index: currentNum ?? "" });
    return () => {
      if (currentNum !== "" && currentNum !== "new") {
        ...
        <>
          <Grid container spacing={2}>
            <Grid xs={12} sm={12} lg={6} xl={6}>
              <TextZoom
                numZoom="MS159"
                nomControle="Num_Demande_Pret"
                label="N° demande"
                valeur={entete?.Num_Demande_Pret}
            ...
            <Grid xs={12} sm={12} lg={6} xl={6}>
              <TextBox
                nomControle="Montant_Pret"
                label="Montant"
                type="number"
                valeur={entete?.Montant_Pret || 0}
                onchange={stateChange}
                style={{ width: "100%" }}
              />
            </Grid>
            <Grid xs={12} sm={12} lg={6} xl={6}>
              <TextBox
                nomControle="Nb_Echeance"
                label="Durée en mois"
                type="number"
                valeur={entete?.Nb_Echeance || 0}
                onchange={stateChange}
                style={{ width: "100%" }}
              />
            </Grid>
            <Grid xs={12} sm={12} lg={6} xl={6}>
              <CalendarZoom
                nomControle="Premiere_Echeance"
                label="1ère échéance"
                valeur={entete?.Premiere_Echeance || new Date()}
                onchange={stateChange}
                sx={{
                  width: "100%",
                  "& input": { fontSize: { xs: "0.85em", sm: "1em" } },
...
export default Demande_Pret;
type TEntete = {
  Num_Demande_Pret: string;
  Matricule?: string;
  Dat_Demande?: Date;
  Montant_Pret?: number;
  Commentaire?: string;
